...
class DPBackSubstitution:
    ...
    def _get_lb(self, expr_w, expr_b):
        ...
        if len(self.lexpr[0].size()) == 2:
            res_w = (
                positive_only(expr_w) @ self.lexpr[0]
                    + negative_only(expr_w) @ self.uexpr[0]
            )
        else:
            res_w = (
                positive_only(expr_w) * self.lexpr[0]
        ...
        res_b = (
            positive_only(expr_w) @ self.lexpr[1]
            + negative_only(expr_w) @ self.uexpr[1]
            + expr_b
        )

        if self.prev_layer == None:
            return (
                positive_only(res_w) @ self.input_dp[0]
                + negative_only(res_w) @ self.input_dp[1]
                + res_b
            )
        else:
    ...
    def _get_ub(self, expr_w, expr_b):
        if len(self.lexpr[0].size()) == 2:
            res_w = (
                positive_only(expr_w) @ self.uexpr[0]
                + negative_only(expr_w) @ self.lexpr[0]
            )
        else:
            res_w = (
                positive_only(expr_w) * self.uexpr[0]
        ...
        res_b = (
            positive_only(expr_w) @ self.uexpr[1]
            + negative_only(expr_w) @ self.lexpr[1]
            + expr_b
        )

        if self.prev_layer == None:
            return (
                positive_only(res_w) @ self.input_dp[1]
                + negative_only(res_w) @ self.input_dp[0]
                + res_b
            )
        else:
...
class LSTMCell(nn.LSTM, DPBackSubstitution):
    ...
    def forward(self, x_t_dp,init_hidden=None):
        ...
        #     device=dev,
        # )
        self.lexpr=(lexpr_w_h, lexpr_b_h)
        self.uexpr=(uexpr_w_h, uexpr_b_h)
        self.initialised = True

        return self.h_t_lb,self.h_t_ub
...
class Linear(nn.Linear, DPBackSubstitution):
    ...
    def forward(self, prev_dp):
        ...
        if self.prev_layer == None:
            ...
            lb = (
                positive_only(self.weight) @ self.input_dp[0]
        ...
        self.lexpr=(self.weight, self.bias)
        self.uexpr=(self.weight, self.bias)
        # self.output_dp = DeepPoly(
        #     lb=lb,
        #     ub=ub,
...
class Convolution_Layer(nn.Conv2d, DPBackSubstitution):
    ...
    def convert(layer, height, width, prev_layer=None, device=torch.device("cpu")):
        ...
        stride=layer.stride
        pad=layer.padding
        weight=layer.weight.data.to('cpu')
        bias=layer.bias.data.to('cpu')
...
class MaxPool(nn.MaxPool2d, DPBackSubstitution):
    def __init__(self, channel,height,width,out_shape,kernel=(2,2),stride=(2,2), prev_layer=None,device=torch.device("cpu")):
    ...
    def convert(layer,channel,h,w, prev_layer=None, device=torch.device("cpu")):
        ...
        out_shape=(int(((h-kernel[0])/stride[0])+1),
                   ...
                   channel)
        return MaxPool(channel,h,w,out_shape,kernel,stride,prev_layer,device)

    def forward(self, prev_dp):
        dim = prev_dp[0].size()[0]
        #input flattened from (h,w,c)
        out_dim=np.prod(self.out_shape)

        lexpr_w = torch.zeros(dim,out_dim).to(device=self.dev)
        lexpr_b = torch.zeros(out_dim).to(device=self.dev)
        uexpr_w = torch.zeros(dim,out_dim).to(device=self.dev)
        uexpr_b = torch.zeros(out_dim).to(device=self.dev)
                ...
                for z in range(f):
                    ...
                    for idx0 in range(self.kernel[0]):
                        for idx1 in range(self.kernel[1]):
                            ...
                            tempub[n*p*i+p*j+z]=prev_dp[1][n*p*i+p*j+z]
                            templb[n*p*i+p*j+z]=prev_dp[0][n*p*i+p*j+z]
...
class BatchNormalization(nn.BatchNorm2d,DPBackSubstitution):
    ...
    """
    def __init__(self, in_channel, mean, var, prev_layer=None,device=torch.device("cpu")):
    ...
    def forward(self, prev_dp):
        ...
        lexpr_w = torch.zeros(first_dim,self.in_channel).to(device=self.dev)
        lexpr_b = torch.zeros(first_dim,self.in_channel).to(device=self.dev)
        ...
        self.uexpr = (lexpr_w, lexpr_b)

        return lb, ub


class Selection(nn.ReLU,DPBackSubstitution):
    def __init__(self,idx, prev_layer=None,device=torch.device("cpu")):
    ...
    def forward(self, prev_dp):
        dim = prev_dp[0].size()[0]
        lexpr_w = torch.zeros(dim,len(self.idx)).to(device=self.dev)
        lexpr_b = torch.zeros(len(self.idx)).to(device=self.dev)
...
class Add(nn.ReLU,DPBackSubstitution):
    ...
    def forward(self, prev_dp, add_term):
        dim = prev_dp[0].size()[0]
        lexpr_w = torch.ones(dim).to(device=self.dev)
        lexpr_b = torch.tensor(add_term).to(device=self.dev)
...
class ReLU(nn.ReLU, DPBackSubstitution):
    def __init__(self, inplace=False, prev_layer=None,device=torch.device("cpu")):
    ...
    def forward(self, prev_dp):
        ...
        lexpr_b = torch.zeros(dim).to(device=self.dev)
        uexpr_w = torch.zeros(dim).to(device=self.dev)
        uexpr_b = torch.zeros(dim).to(device=self.dev)
        ...
        # lb = self.prev_layer._get_lb(torch.diag(lexpr_w), lexpr_b)
        # ub = self.prev_layer._get_ub(torch.diag(uexpr_w), uexpr_b)
        lb=torch.zeros(dim).to(device=self.dev)
...
class Sigmoidal(nn.Sigmoid, DPBackSubstitution):
    def __init__(self, func, prev_layer=None,device=torch.device("cpu")):
    ...
    def forward(self, prev_dp):
        dim = prev_dp[0].size()[0]
        lexpr_w = torch.zeros(dim).to(device=self.dev)
        lexpr_b = torch.zeros(dim).to(device=self.dev)
        uexpr_w = torch.zeros(dim).to(device=self.dev)
        uexpr_b = torch.zeros(dim).to(device=self.dev)
...
class Exponential(nn.ReLU, DPBackSubstitution):
    ...
    def forward(self, prev_dp):
        ...
        delta = torch.Tensor([1e-2]).to(device=self.dev)
