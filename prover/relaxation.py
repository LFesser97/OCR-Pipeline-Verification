...
def ibp_bounds(lx, ux, ly, uy, tanh=True):
    ...
    candidates = torch.Tensor(
        [lstm_f(lx, ly, tanh), lstm_f(lx, uy, tanh), lstm_f(ux, ly, tanh), lstm_f(ux, uy, tanh)]
    )
...
def get_LB_delta(Al, Bl, Cl, lx, ux, ly, uy, tanh):
    # Cl calibration.
    bndX = np.array([lx, lx, ux, ux])# four corners
    ...
    delta = np.min(lstm_f(bndX, bndY, tanh) - Al * bndX - Bl * bndY - Cl)
    return delta
...
def get_UB_delta(Au, Bu, Cu, lx, ux, ly, uy, tanh):
    ...
    delta = np.min(Au * bndX + Bu * bndY + Cu - lstm_f(bndX, bndY, tanh))
    return delta
...
def LB(lx, ux, ly, uy, tanh=True, n_samples=100):
    ...
    model.addConstrs(
        (Al * X[i] + Bl * Y[i] + Cl <= lstm_f(X[i], Y[i], tanh) for i in range(n_samples)),
        name="ctr",
    )

    obj = LinExpr()
    obj = np.sum(lstm_f(X, Y, tanh)) - Al * np.sum(X) - Bl * np.sum(Y) - Cl * n_samples
...
def UB(lx, ux, ly, uy, tanh=True, n_samples=100):
    ...
    model.addConstrs(
        (Au * X[i] + Bu * Y[i] + Cu >= lstm_f(X[i], Y[i], tanh) for i in range(n_samples)),
        name="ctr",
    )

    obj = LinExpr()
    obj = Au * np.sum(X) + Bu * np.sum(Y) + Cu * n_samples - np.sum(lstm_f(X, Y, tanh))
    model.setObjective(obj, GRB.MINIMIZE)
...
def LB_split(lx, ux, ly, uy, tanh=True, split_type=0, n_samples=200):
    ...
    model.addConstrs(
        (Al * X[i] + Bl * Y[i] + Cl <= lstm_f(X[i], Y[i], tanh) for i in range(n)),
        name="ctr",
    )

    obj = LinExpr()
    obj = np.sum(lstm_f(X, Y, tanh)) - Al * np.sum(X) - Bl * np.sum(Y) - Cl * n
    if split_type == 11:
        obj -= lstm_f(lx, uy, tanh) - Al * lx - Bl * uy - Cl
    elif split_type == 12:
        obj -= lstm_f(ux, ly, tanh) - Al * ux - Bl * ly - Cl
    elif split_type == 21:
        obj -= lstm_f(ux, uy, tanh) - Al * ux - Bl * uy - Cl
    elif split_type == 22:
        obj -= lstm_f(lx, ly, tanh) - Al * lx - Bl * ly - Cl
    model.setObjective(obj, GRB.MINIMIZE)
...
def UB_split(lx, ux, ly, uy, tanh=True, split_type=0, n_samples=200):
    ...
    model.addConstrs(
        (Au * X[i] + Bu * Y[i] + Cu >= lstm_f(X[i], Y[i], tanh) for i in range(n)),
        name="ctr",
    )

    obj = LinExpr()
    obj = Au * np.sum(X) + Bu * np.sum(Y) + Cu * n - np.sum(lstm_f(X, Y, tanh))
    if split_type == 11:
        obj += lstm_f(lx, uy, tanh) - Au * lx - Bu * uy - Cu
    elif split_type == 12:
        obj += lstm_f(ux, ly, tanh) - Au * ux - Bu * ly - Cu
    elif split_type == 21:
        obj += lstm_f(ux, uy, tanh) - Au * ux - Bu * uy - Cu
    elif split_type == 22:
        obj += lstm_f(lx, ly, tanh) - Au * lx - Bu * ly - Cu
    model.setObjective(obj, GRB.MINIMIZE)
...
#
# def attn_f(xs,ys):
#     return np.dot(softmax(xs),ys)
#
# def attn_get_LB_delta(Al, Bl, Cl, lx, ux, ly, uy, tanh):
#     # Cl calibration.
#     bndX = np.array([lx, lx, ux, ux])# four corners
#     bndY = np.array([ly, uy, ly, uy])
#     if not tanh:
...
#
#
# def attn_get_UB_delta(Au, Bu, Cu, lx, ux, ly, uy, tanh):
#     # Cu calibration.
#     bndX = np.array([lx, lx, ux, ux])
#     bndY = np.array([ly, uy, ly, uy])
#     if not tanh:
...
#
#
# def attn_LB(lx, ux, ly, uy, tanh=True, n_samples=100):
#     # Calculate Al, Bl, Cl by sampling and linear programming.
#     bndX = np.array([lx, lx, ux, ux])
#     bndY = np.array([ly, uy, ly, uy])
#     X = np.concatenate([bndX, np.random.uniform(lx, ux, n_samples - 4)])
...
#     if model.status == GRB.Status.OPTIMAL:
#         Al, Bl, Cl = model.getAttr("x", model.getVars())
#         delta = attn_get_LB_delta(Al, Bl, Cl, lx, ux, ly, uy, tanh)
#         Cl += delta
#         return Al, Bl, Cl
#     else:
#         return None, None, None
#
#
# def attn_UB(lx, ux, ly, uy, tanh=True, n_samples=100):
#     # Calculate Au, Bu, Cu by sampling and linear programming.
#     bndX = np.array([lx, lx, ux, ux])
#     bndY = np.array([ly, uy, ly, uy])
#     X = np.concatenate([bndX, np.random.uniform(lx, ux, n_samples - 4)])
...
#     if model.status == GRB.Status.OPTIMAL:
#         Au, Bu, Cu = model.getAttr("x", model.getVars())
#         delta = attn_get_UB_delta(Au, Bu, Cu, lx, ux, ly, uy, tanh)
#         Cu -= delta
#         return Au, Bu, Cu
#     else:
#         return None, None, None
#
#
# def attn_bounds(lx, ux, ly, uy, tanh=True):
#     # Caller function to obtain lower and upper bounding planes.
#     if type(lx) is torch.Tensor:
#         lx = lx.item()
#     if type(ux) is torch.Tensor:
...
#     if type(uy) is torch.Tensor:
#         uy = uy.item()
#     Al, Bl, Cl = attn_LB(lx, ux, ly, uy, tanh)
#     Au, Bu, Cu = attn_UB(lx, ux, ly, uy, tanh)
#     return Al, Bl, Cl, Au, Bu, Cu
